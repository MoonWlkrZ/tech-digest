...
def main():
    ...
    for i in sorted(items,key=lambda x:(-(x.get('rough_score') or x.get('score') or 0),str(x.get('published',''))),reverse=False):
        ...
        hl = i.get('highlight')
        hl_html = f'<p class="highlight">{html.escape(hl)}</p>' if hl else ''
        item_html.append(f'''<article class="item"><div class="source">{html.escape(i.get('source',''))} <span class="score">{stars(score)}</span></div><h3><a href="{html.escape(i.get('url',''))}" target="_blank" rel="noopener noreferrer">{html.escape(i.get('title',''))}</a></h3><p class="summary">{html.escape(i.get('summary') or '打开原文阅读。')}</p>{hl_html}<div class="card-foot"><span>{html.escape(str(i.get('published',''))[:16])}</span><span>↗ 原文</span></div></article>''')
