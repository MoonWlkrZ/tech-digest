...
def main():
    ...
    item_html=[]
    # 阅读队列排序:带粗读 highlight 的卡片优先(避免 highlight 断层),组内按 rough_score 降序、发布时间升序
    for i in sorted(items,key=lambda x:(0 if x.get('highlight') else 1,-(x.get('rough_score') or x.get('score') or 0),str(x.get('published',''))),reverse=False):
        score=i.get('rough_score',i.get('score'))
        ex = i.get('excerpt') or i.get('summary') or ''
        hl = i.get('highlight')
        body_html = ''
        if ex:
            body_html += f'<p class="excerpt">{html.escape(ex)}</p>'
        if hl:
            body_html += f'<p class="summary">{html.escape(hl)}</p>'
        item_html.append(f'''<article class="item{' has-hl' if hl else ''}"><div class="source">{html.escape(i.get('source',''))} <span class="score">{stars(score)}</span></div><h3><a href="{html.escape(i.get('url',''))}" target="_blank" rel="noopener noreferrer">{html.escape(i.get('title',''))}</a></h3>{body_html}<div class="card-foot"><span>{html.escape(str(i.get('published',''))[:16])}</span><span>↗ 原文</span></div></article>''')
