...
def main():
    ...
    hl_items=sorted([i for i in items if i.get('highlight')],key=_qkey)
    rest_items=sorted([i for i in items if not i.get('highlight')],key=_qkey)
    ...
    html_doc=f'''<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="description" content="AI-curated technology reading digest"><title>Tech Brief — 技术阅读日报</title><style>{CSS}</style></head><body><main class="shell"><div class="topline"><div class="brand">Tech Brief / Reading Desk</div><div class="date">edition / {html.escape(str(data.get('edition','')))}</div></div><header><div><div class="source">AI-assisted daily reading</div><h1>值得读的技术，<br>不只是收到的技术。</h1><p class="dek">先聚合，再筛选；AI 读完少数真正重要的文章，给出推荐顺序和完整判断。</p></div><aside class="note">{len(recs)} 篇完整精读<br>{len(hl_items)} 篇已粗读（带正文）<br>{len(rest_items)} 篇标题级初筛<br>来源只作为辅助标签</aside></header><section class="section"><div class="section-head"><h2>今日推荐顺序</h2><span class="section-count">{len(recs)} selected</span></div><div class="recommendations">{''.join(rec_html)}</div></section><section class="section"><div class="section-head"><h2>今日判断</h2></div><p class="judgment">{html.escape(data.get('judgment',''))}</p></section><section class="section"><div class="section-head"><h2>阅读队列</h2><span class="section-count">{len(hl_items)} 篇已粗读 · 单列整行 · 正文全文</span></div><div class="queue">{''.join(hl_html)}</div><div class="section-head" style="margin-top:34px"><h2>其余候选</h2><span class="section-count">{len(rest_items)} 篇 · 仅标题级初筛分</span></div><div class="grid">{''.join(rest_html)}</div></section><footer><span>原文链接指向文章作者/发布站点</span><span>Generated from structured digest data</span></footer></main></body></html>'''
